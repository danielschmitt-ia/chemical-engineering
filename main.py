...
if __name__ == "__main__":
    ...
    axs[0].set_title('Fuga Térmica (Runaway) — integração RK45')
    axs[0].legend(); axs[0].grid(True)

    axs[1].plot(t2, hist_T_mpc, color='purple', label='T (MPC)')
    axs[1].axhline(y=reator.T_alvo, color='black', linestyle=':', label='Setpoint')
    axs[1].axhline(y=reator.T_max_seguro, color='red', linestyle='--', label='Teto de segurança')
    axs[1].set_title('MPC com restrições de segurança (temperatura e taxa do atuador)')
    axs[1].legend(); axs[1].grid(True)

    axs[2].plot(t2, hist_CA_mpc, label='Real')
    axs[2].plot(t2, concentracao_predita, '--', label='IA (soft sensor)')
    axs[2].set_title('Soft Sensor (Rede Neural)')
    axs[2].legend(); axs[2].grid(True)

    axs[3].plot(t3, UA_real_falha, color='darkorange', label='UA real (fouling)')
    axs[3].axhline(y=50000.0, color='gray', linestyle=':', label='UA nominal do modelo')
    if tempo_deteccao is not None:
        axs[3].axvline(x=tempo_deteccao, color='red', linestyle='--', label='Falha detectada')
    axs[3].set_title('Degradação simulada do coeficiente de troca térmica (UA)')
    axs[3].legend(); axs[3].grid(True)

    axs[4].plot(t3, residuo_falha, color='crimson', label='Resíduo (EWMA)')
    axs[4].axhline(y=0.4, color='black', linestyle=':', label='Limiar de alarme')
    if tempo_deteccao is not None:
        axs[4].axvline(x=tempo_deteccao, color='red', linestyle='--')
    axs[4].set_title('Detecção de Falha por Resíduo do Gêmeo Digital')
